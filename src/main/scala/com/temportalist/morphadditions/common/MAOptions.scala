...
import com.temportalist.origin.api.common.utility.{Json, ThreadFetchOnlineFile}
...
object MAOptions extends OptionRegister {
	...
	private var morphAdditionConfigDir: File = null
	...
	override def customizeConfiguration(event: FMLPreInitializationEvent): Unit = {
		this.morphAdditionConfigDir = this.getConfigDirectory(event.getModConfigurationDirectory)
		val mapHelper = new File(this.morphAdditionConfigDir, "MappingHelp.txt")
		if (!mapHelper.exists()) {
			val text: String =
	...
	def postInitAbilities(): Unit = {
		val mapAbilities: File = new File(this.morphAdditionConfigDir, "MapAbilities.json")
		var onlineAbilitiesString = this.fetchOnlineAbilitiesAndWait





		var onlineAbilities: JsonObject =
			if (onlineAbilitiesString != null)
				Json.parser.parse(onlineAbilitiesString).getAsJsonObject
			else null
		...
		if (!mapAbilities.exists()) {
			try {
				if (onlineAbilitiesString == null) {
					onlineAbilitiesString = Json.toReadableString(this.getDefaultAbilities.toString)
					MorphAdditions.log(
						"Could not load resources from the cloud. Defaulting to previous ability mappings.")
				}
		...
		if (mapAbilities.exists()) {
			...
			if (onlineAbilities == null) {
				val onlineAbilitiesString = this.fetchOnlineAbilitiesAndWait
				if (onlineAbilitiesString != null)
	...
	private def fetchOnlineAbilitiesAndWait(): Unit = {
		this.fetchResource("https://raw.githubusercontent.com/TheTemportalist/"
				+ "MorphAdditions/master/src/main/"
				+ "resources/assets/morphadditions/DefaultAbilities.json", (fileString: String) => {
			this.loadAndRegisterAbilities()
		})
	}

	// todo move this to ThreadFetchOnlineFile
	def fetchResource[U](url: String, returnFunction: String => U): Unit = {
		if (url == null || returnFunction == null) throw new NullPointerException
		new ThreadFetchOnlineFile(url, returnFunction).start()
	}
	...
	*/
	...
	private def getDefaultAbilities: JsonObject = {
		val mcObject = new JsonObject
		mcObject.addProperty(
			"net.minecraft.entity.passive.EntityChicken",
			"drop|6000,12000|minecraft:egg:0"
		)
		...
		jsonObject.add("net.minecraft", mcObject)
		jsonObject
	}
